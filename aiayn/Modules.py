...
class MultiHeadAttention(nn.Module):
    def __init__(self, model_dim, num_heads, k_dim, v_dim, dropout):
        ...
        self.temperature = self.scale = torch.sqrt(torch.FloatTensor([self.head_dim]))
    ...
    def forward(self, q, k, v src_mask):
        ...
        attention_values = torch.matmul(q / self.temperature, k.transpose(2, 3))
        attention_values = attention_values.masked_fill(src_mask == 0, -1e9)
        attn = self.dropout(F.softmax(attention_values, dim=-1))
