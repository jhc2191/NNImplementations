...
class Decoder(nn.Module):
    ...
    def forward(self, trg, encoder_output, src_mask, trg_mask):
        ##same construction as encoder
        trg = self.dropout(self.token_embedding(trg)*self.scale(trg) + self.position_encoding(trg))
...
class Decoder_Layer(nn.Module):
    ...
    def forward(self, decoder_input, encoder_output, src_mask, trg_mask):
        ##pass through the three decoder levels
        ...
        decoder_output_non_residual, _ = self.encoder_attention(encoder_output, decoder_input, encoder_output, trg_mask)
